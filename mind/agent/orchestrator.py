...
async def process(
    ...
    if schema_description:
        active_system_prompt += (
            ...
            "3. Usa SOLO los nombres de columna exactos del esquema (total_amount, "
            "created_at, user_id, etc.). No los traduzcas al español ni uses alias con "
            "comillas dobles.\n"
            "4. Si una consulta falla, intenta una versión más simple (menos JOINs, "
            ...
            "- Productos vendidos: SELECT p.name || ' - ' || pv.variant_name AS producto, "
            "SUM(oi.quantity) FROM "
            "order_items oi JOIN product_variants pv ON oi.product_variant_id = pv.variant_id "
            "JOIN products p ON pv.product_id = p.product_id "
            "GROUP BY p.name, pv.variant_name ORDER BY SUM(oi.quantity) DESC\n"
            "- Ítems de una orden: SELECT p.name || ' - ' || pv.variant_name AS producto, "
            "oi.quantity, oi.unit_price, oi.total_price FROM order_items oi "
            "JOIN product_variants pv ON oi.product_variant_id = pv.variant_id "
            "JOIN products p ON pv.product_id = p.product_id "
            "WHERE oi.order_id = <ID> ORDER BY producto\n"
            ...
            "ÓRDENES CONTRA ENTREGA (COD):\n"
            "- Las órdenes 'contra entrega' o 'pago al recibir' están en la tabla 'cod_orders'. "
            "Para consultar el total a cobrar o lo recolectado, haz JOIN entre cod_orders y "
            "cod_settlements.\n"
            "- También puedes identificar órdenes contra entrega en la tabla 'orders' cuando "
            "notes contiene 'Pago Contra Entrega'.\n\n"
